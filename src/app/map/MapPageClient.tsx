...
export default function MapPageClient({
  ...
  return (
    <>
      ...
      {/* Breadcrumb + compact filter bar */}
      <div className="w-full bg-slate-50 border-b border-slate-200 px-4 py-2 sticky top-16 z-30 relative flex items-center justify-between" ref={filterRef}>
        <div className="flex items-center gap-2 text-xs font-medium text-slate-500">
          <Link href="/" className="flex items-center gap-1 hover:text-[#0044FF] transition-colors">
            <Home className="h-3 w-3" />
            <span>Home</span>
          </Link>
          <ChevronRight className="h-3 w-3 text-slate-300" />
          <span className="text-slate-700 font-semibold">Real Estate</span>
          <ChevronRight className="h-3 w-3 text-slate-300" />
          <span className="text-slate-700">Map</span>
        </div>

        <div className="flex items-center gap-2">
          <div className="relative flex items-center min-w-[120px] max-w-[180px]">
            <Search className="absolute left-2.5 h-3 w-3 text-gray-400" />
            <input
              type="text"
              placeholder="Area..."
              className="w-full pl-7 pr-2 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:ring-2 focus:ring-blue-500 outline-none transition"
              value={searchArea}
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
              isFilterOpen
                ...
                : 'bg-white text-gray-700 border-gray-200 hover:bg-blue-50 hover:text-[#0044FF]'
            }`}
          >
        ...
        {/* Dropdown filter tray */}
        {isFilterOpen && (
          <div className="absolute left-0 right-0 top-full mt-0 mx-4 bg-white rounded-xl shadow-xl border border-gray-200 p-4 z-50">
      ...
      <div className="flex flex-col md:flex-row h-[calc(100vh-64px)] w-full overflow-hidden relative bg-white">

        {/* LEFT FEED PANEL */}
        <aside className={`${isMobileMapActive ? 'hidden' : 'flex flex-col'} md:flex md:flex-col w-full md:w-[450px] lg:w-[500px] h-full overflow-y-auto z-10 border-r border-gray-200`}>
          {!selectedProperty ? (
        ...
        {/* RIGHT MAP PANEL */}
        <div className={`${isMobileMapActive ? 'block' : 'hidden'} md:block w-full md:flex-1 h-full relative z-20`}>
          <SafeLeafletMap
